...
pub const ObjectStore = struct {
    ...
    /// oid -> location for loose objects and pack entries.
    ...
    pub fn open(allocator: std.mem.Allocator, repo: *const Repository) StoreError!ObjectStore {
        ...
        store.loose = loose_mod.scan(allocator, objects_dir) catch |err| switch (err) {
            error.OutOfMemory => return error.OutOfMemory,
            error.CorruptRepository => return error.CorruptRepository,
            else => return error.Unexpected,
        };
        for (store.loose.objects.items, 0..) |*o, i| {
            try store.locations.put(allocator, o.id, .{ .loose = i });
        }
    ...
    fn openPacks(self: *ObjectStore, objects_dir: []const u8) StoreError!void {
        ...
        while (it.next(io()) catch return error.Unexpected) |ent| {
            ...
            const pack_id = self.packs.items.len;
            ...
            var i: usize = 0;
            while (i < p.index.count) : (i += 1) {
                const id = p.index.oidAt(i);
                const gop = try self.locations.getOrPut(self.allocator, id);
                if (!gop.found_existing) {
                    gop.value_ptr.* = .{ .pack = .{ .pack_id = pack_id, .offset = p.index.offsetAt(i) } };
                }
            }
        }
    }
    ...
    pub fn locate(self: *const ObjectStore, id: *const object_id.ObjectId) Located {
        return self.locations.get(id.*) orelse .missing;
    }
    ...
    pub fn info(self: *const ObjectStore, id: *const object_id.ObjectId) StoreError!Info {
        switch (self.locate(id)) {
            ...
            .loose => |i| {
                ...
                return .{ .object_type = o.object_type, .size = o.size };
            },
            .pack => |loc| {
                const pf = &self.packs.items[loc.pack_id];
                const inf = pf.pack.infoAt(loc.offset) catch return error.CorruptRepository;
    ...
    pub fn readPayload(self: *const ObjectStore, allocator: std.mem.Allocator, id: *const object_id.ObjectId) StoreError!Payload {
        switch (self.locate(id)) {
            ...
            .loose => |i| {
                const o = self.loose.objects.items[i];
                var mf = mmap.MappedFile.init(o.path) catch return error.CorruptRepository;
                defer mf.deinit();
                const size: usize = @intCast(o.size);
    ...
    var decomp = std.compress.flate.Decompress.init(&in, .zlib, &window);
                ...
                return .{ .object_type = o.object_type, .data = out };
            },
            .pack => |loc| {
                const pf = &self.packs.items[loc.pack_id];
                const r = pf.pack.readPayload(allocator, loc.offset) catch |err| switch (err) {
                    error.OutOfMemory => return error.OutOfMemory,
                    else => return error.CorruptRepository,
                };
